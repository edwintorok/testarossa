...
module Pool_test = struct
  ...
  let perform ctx _self = function
    ...
    | `ha_enable ->
       rpc ctx @@ Pool.enable_ha ~heartbeat_srs:[] ~configuration:[]
end

let name_label = "create-test A✔ \"'<{[aa;#"
...
module SR_test = struct
  ...
  let perform ctx sr (op: operation) = match op with
    ...
    | `vdi_snapshot -> vdi ()
    | `vdi_introduce ->
       on_self ctx sr SR.get_VDIs >>= fun vdis ->
       Lwt_list.iter_p (fun vdi ->
           on_self ctx vdi @@ VDI.get_record >>= fun vdir ->
           rpc ctx @@ VDI.forget ~vdi >>= fun () ->
           rpc ctx @@ VDI.introduce ~uuid:vdir.API.vDI_uuid ~name_label:vdir.API.vDI_name_label ~name_description:vdir.API.vDI_name_description ~_type:vdir.API.vDI_type ~sharable:vdir.API.vDI_sharable ~read_only:vdir.API.vDI_read_only ~other_config:vdir.API.vDI_other_config ~location:vdir.API.vDI_location ~sm_config:vdir.API.vDI_sm_config ~managed:vdir.API.vDI_managed ~virtual_size:vdir.API.vDI_virtual_size ~physical_utilisation:vdir.API.vDI_physical_utilisation ~metadata_of_pool:vdir.API.vDI_metadata_of_pool ~is_a_snapshot:vdir.API.vDI_is_a_snapshot ~snapshot_time:vdir.API.vDI_snapshot_time ~snapshot_of:vdir.API.vDI_snapshot_of ~sR:sr ~xenstore_data:vdir.API.vDI_xenstore_data >>= fun _ ->
           Lwt.return_unit
         ) vdis
    | `vdi_resize -> vdi ()
...
module VDI_test = struct
  ...
  let perform ctx self (op:operation) = match op with
    ...
    | `enable_cbt ->
       on_self ctx self VDI.enable_cbt
    | `set_on_boot ->
    ...
    | `resize_online ->
       on_self ctx self VDI.get_virtual_size >>= fun size ->
       rpc ctx @@ VDI.resize_online ~vdi:self ~size:(Int64.add size 65536L)
    | `mirror -> todo "Seems unused?"
    | `forget ->
       rpc ctx @@ VDI.forget ~vdi:self
    | `copy -> todo "VDI.copy"
...
module Host_test = struct
  ...
  let perform ctx host = function
    | `evacuate ->
       rpc ctx @@ Host.evacuate ~host
    | `power_on ->
       rpc ctx @@ Host.power_on ~host
    | `provision -> todo "??"
    | `reboot ->
       rpc ctx @@ Host.reboot ~host
    | `shutdown ->
       rpc ctx @@ Host.shutdown ~host
    | `disable ->
