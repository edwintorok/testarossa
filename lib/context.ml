...
module PP = struct
  ...
  let rpc_response ppf r =
    pf ppf "%s: %a"
      ...
      (using Jsonrpc.to_string string)
      r.Rpc.contents
end
...
module Log_rpc = (val (Logs_lwt.src_log src : (module Logs_lwt.LOG)))
...
let wrap_rpc dest f call =
  ...
  let on_reply reply =
    ...
    debug (fun m ->
        m ~header "%d<[+%a] %a" this_id Mtime.Span.pp dt PP.rpc_response reply
    )
    >>= fun () -> Lwt.return reply
  in
  let on_error err =
    let dt = Mtime_clock.count c in
