...
let device_config ?(provider= "iscsi") ~ip ~iqn ?scsiid () =
  ...
  in
  (match scsiid with Some id -> ("ScsiId", `String id) :: conf | None -> conf)
  |> dict |> to_string ~minify:true |> fun x -> [("uri", x)]
...
let probe ctx ~iscsi ~iqn host =
  ...
  Lwt.catch
    (fun () ->
      ...
      @@ SR.probe ~host
           ~device_config:
             [("target", Ipaddr.V4.to_string iscsi); ("targetIQN", iqn)]
           ~_type:"lvmoiscsi" ~sm_config:[] )
    (fun e ->
      match e with
      | Api_errors.Server_error (_, [_; _; xml]) ->
          Lwt.return xml
      | e ->
         ...
         (fun (got_master, prev) ip ->
           let master =
             if Ipaddr.V4.compare prev ip = 0 then
               if got_master <> None && got_master <> Some ip then
                 failwith "multiple pools found"
               else Some ip
             else got_master
           in
           (master, ip) )
